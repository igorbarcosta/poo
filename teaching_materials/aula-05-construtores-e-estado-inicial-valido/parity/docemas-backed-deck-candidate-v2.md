...
## A atribuição não chega ao campo

```java
descricao = descricao;
```

Os dois nomes se referem ao parâmetro. O valor é atribuído ao próprio
parâmetro, e o campo do objeto não é alterado.

```java
this.descricao = descricao;
```

<div class="key-point"><code>this.descricao</code> é o campo do objeto atual; <code>descricao</code> é o parâmetro do construtor.</div>
...
Um código **cliente** — um trecho fora de `ItemPedido` — recebeu `descricao`,
`preco` e `quantidade` e precisa criar o item.
...
## Compare com a classe responsável
...
Considere o construtor que só incorpora preço e quantidade não negativos.

1. O que ocorre se outro cliente esquecer as verificações de A?
2. Qual proposta concentra a regra em todos os pontos de criação?
3. Quem conhece melhor os estados aceitáveis de `ItemPedido`?

<!-- Escolher uma proposta, justificar e contrastar hipóteses antes da formalização. -->
...
Uma hospedagem registra quantas pessoas ficarão e o valor de cada diária.

```java
int quantidadeDePessoas = 4;
double valorDaDiaria = 180.0;

Reserva reserva =
    new Reserva(quantidadeDePessoas, valorDaDiaria);
```

Se essas informações são necessárias para uma `Reserva` fazer sentido, o
estado deve ser estabelecido durante a criação?

Quem deve preservar essa responsabilidade se os argumentos forem inadequados?

<!-- Transferência opcional e elástica. Não implementar Reserva nem definir novas invariantes; fechar coletivamente se houver tempo. -->
