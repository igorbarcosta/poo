...
- Todos os ícones de navegação pertencem à família Lucide. Eles marcam apenas categorias de primeiro nível; itens internos permanecem textuais, sem repetir ícones quando o grupo já fornece contexto semântico. Usar o mapeamento: **Início** — `lucide/house`; **Aulas** — `lucide/book-open`; **Laboratórios** — `lucide/flask-conical`; **Avaliações** — `lucide/clipboard-check`; **Materiais** — `lucide/book-copy`. O ícone discreto do topo representa a identidade conceitual da disciplina sem funcionar como marca gráfica independente.
